...
sys.path.append('../../../')
sys.path.append('../../../../')
from src.architecture_le2.social_partner import SocialPartner
from src import logger
from src.architecture_le2.experiment import config
...
REW_FUNC = 'pretrained'#'learned_lstm'
POLICY_ARCHITECTURE = 'modular_attention'
POLICY_ENCODING = 'lstm'
GOAL_INVENTION = 'from_epoch_500'
...
def train(policy, training_worker, evaluation_worker, data_processor, goal_sampler, eval_goal_sampler, reward_function,
    ...
    for epoch in range(n_epochs):
        ...
        rf_metrics = reward_function.get_metrics_record()
...
if __name__ == '__main__':
    ...
    add('--seed', type=int, default=1)#np.random.randint(int(1e6)), help='the random seed used to seed both the environment and the training code')
    ...
    add('--bias_buffer', default=BIAS_BUFFER, type=lambda x: (str(x).lower() == 'true'), help="whether the sampling of buffer fight unbalanced distributions of goals")
