...
class RewardFunctionLSTMPretrained:
    ...
    def evaluate(self, states_test, state_id_buffer_test):
        ...
        if len(states_test) > 0:
            ...
            # with open('/Users/tristankarch/Repo/gnn-rl/src/data/small_set.pk', 'rb') as fp:
            #     test_set = pickle.load(fp)
            #
            # ss = test_set['s']
            # dd = test_set['d']
            # rr = test_set['r']
            # descriptions = test_set['descr']
            #
            # goal_ids_dataset = []
            # s_dataset = []
            # d_dataset = []
            # r_dataset = []
            # for descr, s, d, r in zip(descriptions, ss, dd, rr):
            #     if descr in self.goal_sampler.feedback2id.keys():
            #         goal_ids_dataset.append(self.goal_sampler.feedback2id[descr])
            #         s_dataset.append(s)
            #         d_dataset.append(d)
            #         r_dataset.append(r)
            #
            ...
            # with torch.no_grad():
            #     predictions = torch.round(self.reward_function(torch.tensor(s_dataset, dtype=torch.float32),
            #                                                    torch.tensor(d_dataset,
            #                                                                 dtype=torch.float32)))
            #     descriptions_embedding = np.atleast_2d(
            #         np.array(self.goal_sampler.feedback_memory['reward_encoding'])[goal_ids_dataset])
            #     predictions_from_emb = torch.round(
            #         self.reward_function.pred_from_h_lstm(torch.tensor(s_dataset, dtype=torch.float32),
            #                                               torch.tensor(descriptions_embedding, dtype=torch.float32)))
